...
argument-hint: "[project-path | lightweight]"
---
...
예시 — Structure Explorer Agent tool 호출:
```
description: "Phase 1: Structure discovery"
subagent_type: "general-purpose"
model: "haiku"
run_in_background: true
prompt: "PROJECT_PATH={path}에서 프로젝트 구조를 분석하라. 기술 스택: {detected_stack}.
  수집 항목: 빌드 설정 파일, 워크스페이스 구성, dev/build 명령어, 수정 금지 파일.
  출력 형식: TECH_STACK: [...] / MONOREPO: true|false / COMMANDS: [...] / DO_NOT_MODIFY: [...]"
```

### 에이전트 1 — Structure Explorer

Agent tool 파라미터: `subagent_type="general-purpose"`, `model="haiku"`, `run_in_background=true`

프롬프트 내용 — 입력: PROJECT_PATH + 감지된 스택. 작업: 프로젝트 빌드 설정 파일(스택별: package.json / build.gradle / Cargo.toml / pyproject.toml / go.mod), 워크스페이스 설정, 빌드 명령어, 수정 금지 파일 수집. 출력 형식: `TECH_STACK`, `MONOREPO`, `COMMANDS`, `DO_NOT_MODIFY` 키로 구조화.
...
프롬프트 내용 — "컴파일 성공 → 런타임 실패" 함정 탐색. 포함 조건: (1)컴파일 성공 (2)런타임 실패 (3)코드만으론 발견 불가 (4)디버깅 30분+. 제외: 컴파일러/린터가 잡는 것, 설정 파일에서 발견 가능, 스타일 선호. LSP 사용 가능 시 `lsp_find_references`로 의심 패턴의 호출 범위를 검증하여 trap 후보 정확도 향상 (선택적). 출력: 각 trap별 `name, consequence, bad_code, good_code, why_hidden`. trap 0건도 유효한 결과이며, `trap_count=0`으로 명시 출력.
...
- trap ≥ 1: 각 trap을 번호 + BAD/GOOD으로 작성
...
각 라운드 후 C1-C7 스코어카드 출력 (형식은 `references/sample-lfos-ui.md` 참조).
**모드별 처리**:
...
- `references/sample-lfos-ui.md`: 실제 프로젝트 적용 사례 (C1-C7 9.0+)
